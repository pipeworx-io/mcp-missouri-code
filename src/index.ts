...
/**
 * Missouri Revised Statutes — state statutes by citation.
 *
 * Answers HTTP 200 for a citation that does not exist, so the section number itself is confirmed in the returned text before the page is treated as a statute.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Missouri Revised Statutes');
}

const BASE = 'https://revisor.mo.gov/main/OneSection.aspx?section=';
const UA = 'pipeworx-mcp-missouri-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'mo_revised_statute',
  description: 'Get the FULL TEXT of a section of the Missouri Revised Statutes — Missouri state law — by citation. "RSMo 565.020" is first degree murder. Returns the statutory text. Keyless. Use for "what does Mo. Rev. Stat. 565.020 say" or to check a statute a case relies on.',
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "565.020" (murder 1) or "577.010" (DWI).' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'mo_revised_statute') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '');
  if (!/^\d+\.\d+$/.test(raw)) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Missouri Revised Statutes citation.`,
      hint: 'Sections look like "565.020" (murder 1) or "577.010" (DWI).' };
  }
  const url = `${BASE}${encodeURIComponent(raw)}`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', section: raw,
      message: `No section ${raw} in the Missouri Revised Statutes (HTTP 404).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  if (!res.ok) throw new Error(`Missouri Revised Statutes returned HTTP ${res.status} for ${raw}`);
  const text = statuteText(await res.text());

  // Confirm the SECTION ITSELF is present rather than trusting the status:
  // several state sites answer 200 with a chrome-only page for a citation that
  // does not exist, and returning that would report "no such law" as the law.
  if (!text.includes(raw)) {
    return { found: false, reason: 'section_not_found', section: raw,
      message: `No section ${raw} in the Missouri Revised Statutes. The site returns a page for unknown citations, so this is read from the content rather than the status code.`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  // Anchor on the section number AT A LINE START, not its first appearance
  // anywhere: these pages mention the citation in nav links and RSS hrefs long
  // before the statute, so indexOf lands in the furniture and the caller reads
  // menu text where the law should be.
  const esc = raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // Skip annotation lines: Wisconsin repeats the section number to head its
  // "History" and "Note" blocks, which sit BEFORE the statute, so a plain
  // line-start match lands on editorial apparatus instead of the law.
  let at = text.search(new RegExp(`^\\s*${esc}[\\s.]+(?!History|Note|Annot|Cross)`, 'm'));
  if (at === -1) at = text.search(new RegExp(`^\\s*${esc}[\\s.]`, 'm'));
  const body = at > -1 ? text.slice(at).trim() : text;
  return {
    citation: `Mo. Rev. Stat. § ${raw}`, section: raw,
    text: body, text_chars: body.length, url,
    source: 'Missouri Revised Statutes, keyless',
    attribution: 'State statutes are public record.',
  };
}
